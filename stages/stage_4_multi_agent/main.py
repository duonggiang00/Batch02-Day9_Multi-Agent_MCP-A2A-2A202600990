    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves data sharing, GDPR, privacy, data breaches'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
...
async def privacy_agent(state: LegalState) -> dict:
    """Agent chuyên về luật bảo vệ dữ liệu cá nhân (GDPR, Privacy)."""
    print("\n  [Node: call_privacy_specialist] Phân tích vấn đề dữ liệu / GDPR...")
    llm = get_llm()
...
Hãy phân tích các vấn đề về privacy và GDPR (nếu có). Trả lời ngắn gọn dưới 200 từ.
"""
